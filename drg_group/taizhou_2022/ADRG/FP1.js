...
export default function FP1_group(record){
    ...
    let adrg_zd1=[];
    let adrg_ss=["37.6101","37.6600x001","37.6800x001","37.6800x002","37.6800x003","37.6800x005","39.9500x004","39.9500x005","39.9500x006","39.9500x007","39.9501","39.9600x002","39.9600x003","96.7101"];
    let adrg_ss1=[];
    let dept_list=[];
    
    if (true && adrg_zd.includes(record.zdList[0]) && record.ssList && adrg_ss.includes(record.ssList[0])){
        putMessage('符合FP1入组条件，匹配规则：主诊断匹配、主手术匹配');
        let drg=new MDCF_DRG();
        
        if (drg.FP11_group(record)){
            return 'FP11';
        }

        if (drg.FP13_group(record)){
            return 'FP13';
        }

        if (drg.FP15_group(record)){
            return 'FP15';
        }
