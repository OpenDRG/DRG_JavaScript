...
export default function GK2_group(record){
    ...
    if (true && record.ssList && adrg_ss.includes(record.ssList[0]) && record.ssList && intersect(record.ssList,adrg_ss)){
        ...
        if (drg.GK23_group(record)){
            return 'GK23';
        }

        if (drg.GK25_group(record)){
            return 'GK25';
        }
