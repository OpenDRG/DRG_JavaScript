...
export default function PK1_group(record){
    ...
    let adrg_ss2=[];
    let dept_list=[];
    if (true && record.ssList && adrg_ss.includes(record.ssList[0]) && record.ssList && intersect(record.ssList,adrg_ss) && record.ageDay<=28){
        putMessage('符合PK1入组条件，匹配规则：主手术匹配、某一手术匹配、新生儿');
