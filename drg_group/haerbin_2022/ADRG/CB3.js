import {putMessage,intersect,SS_VALID} from '../Base.js';
import {MDCC_DRG} from '../DRG/MDCC_DRG.js';

export default function CB3_group(record){
    let adrg_zd=[];
    let adrg_zd1=[];
    let adrg_ss=["13.0100","13.0201","13.1100","13.1900x006","13.1900x007","13.1900x008","13.1901","13.1902","13.2x01","13.3x00x001","13.3x01","13.4100x001","13.4101","13.4200x001","13.4300x001","13.5100","13.5900x001","13.6400x001","13.6500x002","13.6501","13.6502","13.6503","13.6600","13.6900x002","13.6901","13.7000","13.7100x001","13.7200x001","13.8x00x003","13.9000x004","13.9000x005","13.9000x006","13.9000x007","13.9000x008","13.9000x009","13.9000x010","13.9000x011","13.9001","13.9002","13.9003","13.9100x001"];
    let adrg_ss1=[];
    let adrg_ss2=[];
    let dept_list=[];
    if (true && record.ssList && adrg_ss.includes(record.ssList[0]) && record.ssList && intersect(record.ssList,adrg_ss)){
        putMessage('符合CB3入组条件，匹配规则：主手术匹配、某一手术匹配');
        let drg=new MDCC_DRG();
        
        if (drg.CB31_group(record)){
            return 'CB31';
        }

        if (drg.CB33_group(record)){
            return 'CB33';
        }

        if (drg.CB35_group(record)){
            return 'CB35';
        }

        return 'CB3';
    }else{
        return '';
    }
}
  