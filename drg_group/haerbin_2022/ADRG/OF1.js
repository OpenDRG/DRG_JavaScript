...
export default function OF1_group(record){
    ...
    let adrg_zd1=[];
    let adrg_ss=["69.0200x003","73.1x01","73.1x02","74.9100","74.9100x001","74.9101","75.0x01","75.0x01x001","75.0x02"];
    let adrg_ss1=[];
    let adrg_ss2=[];
    let dept_list=[];
    if (true && adrg_zd.includes(record.zdList[0]) && record.ssList && adrg_ss.includes(record.ssList[0]) && record.ssList && intersect(record.ssList,adrg_ss)){
        putMessage('符合OF1入组条件，匹配规则：主诊断匹配、主手术匹配、某一手术匹配');
        let drg=new MDCO_DRG();
        
        if (drg.OF19_group(record)){
            return 'OF19';
        }
