...
export default function TS2_group(record){
    ...
    if (true && adrg_zd.includes(record.zdList[0]) && (!record.ssList || !SS_VALID.includes(record.ssList[0]))){
        putMessage('符合TS2入组条件，匹配规则：主诊断匹配');
        let drg=new MDCT_DRG();
        
        if (drg.TS21_group(record)){
            return 'TS21';
        }

        if (drg.TS23_group(record)){
            return 'TS23';
        }

        if (drg.TS25_group(record)){
            return 'TS25';
        }
