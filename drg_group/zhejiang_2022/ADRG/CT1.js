...
export default function CT1_group(record){
    ...
    if (true && adrg_zd.includes(record.zdList[0])){
        putMessage('符合CT1入组条件，匹配规则：主诊断匹配');
        let drg=new MDCC_DRG();
        
        if (drg.CT11_group(record)){
            return 'CT11';
        }

        if (drg.CT13_group(record)){
            return 'CT13';
        }

        if (drg.CT15_group(record)){
            return 'CT15';
        }
