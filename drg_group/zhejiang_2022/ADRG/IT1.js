...
export default function IT1_group(record){
    ...
    if (true && adrg_zd.includes(record.zdList[0]) && (!record.ssList || !SS_VALID.includes(record.ssList[0]))){
        putMessage('符合IT1入组条件，匹配规则：主诊断匹配');
        let drg=new MDCI_DRG();
        
        if (drg.IT11_group(record)){
            return 'IT11';
        }

        if (drg.IT13_group(record)){
            return 'IT13';
        }

        if (drg.IT15_group(record)){
            return 'IT15';
        }
