...
export default function CS1_group(record){
    ...
    if (true && adrg_zd.includes(record.zdList[0]) && (!record.ssList || !SS_VALID.includes(record.ssList[0]))){
        putMessage('符合CS1入组条件，匹配规则：主诊断匹配');
        let drg=new MDCC_DRG();
        
        if (drg.CS10_group(record)){
            return 'CS10';
        }

        if (drg.CS19_group(record)){
            return 'CS19';
        }
