...
export default function ES3_group(record){
    ...
    if (true && adrg_zd.includes(record.zdList[0])){
        putMessage('符合ES3入组条件，匹配规则：主诊断匹配');
        let drg=new MDCE_DRG();
        
        if (drg.ES33A_group(record)){
            return 'ES33A';
        }

        if (drg.ES31A_group(record)){
            return 'ES31A';
        }

        if (drg.ES31B_group(record)){
            return 'ES31B';
        }

        if (drg.ES35A_group(record)){
            return 'ES35A';
        }

        if (drg.ES35B_group(record)){
            return 'ES35B';
        }

        if (drg.ES33B_group(record)){
            return 'ES33B';
        }
