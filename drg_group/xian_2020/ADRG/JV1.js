...
export default function JV1_group(record){
    ...
    if (true && adrg_zd.includes(record.zdList[0])){
        putMessage('符合JV1入组条件，匹配规则：主诊断匹配');
        let drg=new MDCJ_DRG();
        
        if (drg.JV11_group(record)){
            return 'JV11';
        }

        if (drg.JV13_group(record)){
            return 'JV13';
        }

        if (drg.JV15_group(record)){
            return 'JV15';
        }
