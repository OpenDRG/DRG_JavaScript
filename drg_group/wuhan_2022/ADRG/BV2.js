import {putMessage,intersect,SS_VALID} from '../Base.js';
import {MDCB_DRG} from '../DRG/MDCB_DRG.js';

export default function BV2_group(record){
    let adrg_zd=["E05.900x002+G73.0*","E11.405+G73.0*","G70.000","G70.000x002","G70.000x004","G70.000x005","G70.001","G70.002","G70.003","G70.004","G70.005","G70.006","G70.007","G70.008","G70.100","G70.200","G70.200x004","G70.200x005","G70.201","G70.202","G70.800x001","G70.900","G70.900x001","G70.900x003","G70.901","G70.902","G71.000","G71.000x005","G71.000x006","G71.000x010","G71.000x011","G71.001","G71.002","G71.003","G71.004","G71.005","G71.006","G71.007","G71.100","G71.101","G71.102","G71.103","G71.104","G71.105","G71.106","G71.200","G71.200x002","G71.200x003","G71.200x004","G71.200x005","G71.300","G71.300x001","G71.300x003","G71.301","G71.800","G71.800x002","G71.801","G71.900","G71.900x001","G72.000","G72.100","G72.200","G72.200x001","G72.300","G72.301","G72.302","G72.304","G72.401","G72.402","G72.403","G72.404","G72.800x001","G72.800x002","G72.800x003","G72.800x006","G72.800x007","G72.800x008","G72.800x009","G72.800x010","G72.800x011","G72.800x012","G72.800x013","G72.900","R25.200x001"];
    let adrg_zd1=[];
    let adrg_ss=[];
    let adrg_ss1=[];
    let dept_list=[];
    
    if (true && adrg_zd.includes(record.zdList[0])){
        putMessage('符合BV2入组条件，匹配规则：主诊断匹配');
        let drg=new MDCB_DRG();
                
        if (drg.BV21_group(record)){
            return 'BV21';
        }        if (drg.BV23_group(record)){
            return 'BV23';
        }        if (drg.BV25_group(record)){
            return 'BV25';
        }
        return 'BV2';
    }else{
        return '';
    }
}
  